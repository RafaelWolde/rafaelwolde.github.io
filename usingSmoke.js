...
function setObstacleImage(x, y, reset) {
    var vx = 0.0;
    var vy = 0.0;
    ...
    c.save()
    let tempImage = scene.obstacleImage

    let imageData = drawOnNew(tempImage, cX(x), cY(y), canvas.width, canvas.height, f.numX, f.numY)
    ...
    for (var j = 1; j < f.numY - 2; j++) {
        for (var i = 1; i < f.numX - 2; i++) {
            ...
            dx = (i + 0.5) * f.h - x;
            dy = (j + 0.5) * f.h - y
            let index = ((f.numY - j + 3) * f.numX + (i - 2)) * 4
...
image.onload = () => {
    scene.obstacleImage = image
    setupScene(1);
    update();
}


function drawOnNew(image, x, y, width, height, numX, numY) {
    const tempCanvas = document.createElement('canvas')
    const tempContext = tempCanvas.getContext('2d')
    tempCanvas.width = numX
    tempCanvas.height = numY
    let scaleX = numX / width
    let scaleY = numY / height
    let fontSize = 30 * scaleX
    tempContext.lineWidth = 3
    tempContext.font = `${fontSize}px sans-serif`
    tempContext.fontKerning = "auto"
    tempContext.textAlign = "center"
    tempContext.textBaseline = "middle"
    let dup = document.getElementById('dup')
    tempContext.strokeText(dup.textContent, numX / 2, numY / 2 + 2)
    let imagedata = tempContext.getImageData(0, 0, numX, numY)
    return imagedata
}